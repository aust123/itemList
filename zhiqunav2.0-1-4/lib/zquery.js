	...
	$.fn.extend({
		cover: function() {
			if($$.browser.IsIE6) {
				var _obj = this[0];
				if(!_obj) {
					return jQuery.error("cover", "the dom is empty")
				}
				var _data = {},
					_offset = this.offset(),
				...
				_iframe.style.cssText = "background:#FFF;position:absolute;left:" + (_offset.left - _pOffset.left) + "px;top:" + (_offset.top - _pOffset.top) + "px;width:" + _obj.offsetWidth + "px;height:" + _obj.offsetHeight + "px;filter:progid:DXImageTransform.Microsoft.Alpha(opacity=0);opacity:0;" + ((isNaN(_zIndex) ? "" : "z-index:" + (parseInt(_zIndex, 10) - 1)));
		...
		mask: function(pFlag) {
			var _target = this[0],
				_left, _top;
			if(!_target) {
				return $.error("mask", "the Dom object is empty"), this
			}
			this.unmask();
			var _maskObject = {};
			...
			$$.browser.IsIE && (_maskObject.maskIframe = document.createElement("iframe"), _maskObject.maskIframe.style.cssText = c + "filter:progid:DXImageTransform.Microsoft.Alpha(opacity=0);opacity:0;", $(_maskObject.maskIframe).insertBefore(_maskObject.maskDiv));
			this.data("__mask__", _maskObject);
			return this
		},
		unmask: function() {
			if(!this[0]) {
				return $.error("unmask", "the Dom object is empty"), this
			}
				...
				fire: function(context, args) {
					if(this.stack) {
						if(this.firing) {
							...
							if(!(this.flags.once && this.memory)) {
...
(function(f) {
	var h = f.console || {};
	var c = ["assert", "clear", "count", "debug", "dir", "dirxml", "exception", "error", "group", "groupCollapsed", "groupEnd", "info", "log", "profile", "profileEnd", "table", "time", "timeEnd", "timeStamp", "trace", "warn"];
	var b = {
		version: "0.1.0"
	};
	var e;
	for(var d = 0, a = c.length; d < a; d++) {
		...
		b[e] = function(g) {
			return function() {
				...
				Function.prototype.apply.call(h[g], h, arguments)
			}
		}(e)
	}
	f.console = b
