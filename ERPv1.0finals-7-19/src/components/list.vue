    ...
    <div class="subcontent">
             ...
             <section class="orderLists" v-for="(item,index) in dataList" :key='index'>
                <p class="order-list-title">
                  ...
                  <template v-if="otype==2">
                    <span v-if="item.order_state==1" class="status">待确认</span>
                    <span v-if="item.order_state==2" class="status">待质检</span>
                    <span v-if="item.order_state==3" class="status">待结算</span>
                    <span v-if="item.order_state==4" class="status">待入库</span>
                    <span v-if="item.order_state==5" class="status">待付款</span>
                    <span v-if="item.order_state==6" class="status">已完成</span>
                    <span v-if="item.order_state==7" class="status">已取消</span> 
                  </template>                                   
                </p>
                <p class="order-list-info"><img src="../assets/imgs/orderdetail_img.png" alt=""><b>{{item.product_stage}}</b><b>{{item.product_name}}</b>
                <!-- <b>{{item.product_weight}}</b><b class="unit">吨</b> -->
                ...
                <p class="order-list-option">
                  <!-- 仅在销售订单已发货状态时显示 上传 按钮 -->
                  <template v-if="otype==1">
                     <router-link :to="{path:'/settlements',query:{oid:item.order_id,otype:1,path:$route.fullPath}}" v-if="item.order_state==4" class="order-sure jiesuan">上传结算单</router-link>
                  </template>                   
                    <!-- 仅在待确认状态时显示 确定、取消 按钮 -->                    
                    <template v-if="item.order_state==1" v-show="optionEnd">
