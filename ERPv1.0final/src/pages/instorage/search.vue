        ...
        <section class="search-history">
            <template v-if="history.length>0">
              <div class="history-content"  v-show="!isInput">              
                ...
                <p v-for="(item,index) in history" :key="index" @click.self="upSearchHistory(item)">{{item}}<img @click.self="remove(index,item.list_id)" src="../../assets/imgs/sellorder_key_cancel.png" alt=""></p>
              </div>
            </template>
             <template v-else>
            ...
            <template v-if="querydata.length>0">          
                <p class="search-list" v-for="(item,index) in querydata" :key="index" v-html="item" @click="upSearchHistory(item)"></p>            
           </template>
            <template v-else >             
...
export default {
  ...
  methods: {
    ...
    removeData(isAll, i) {
      ...
      api.clearHistorySearchList(params).then(res => {
        console.log(res);
        if (res.status_code == 10000) {
          this.history = isAll
            ? []
    ...
    toList($item) {   
      this.cookie.setCookie('keywords',$item,10);
