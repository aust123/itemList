...
$('.navbar a').click(function() {
	...
	if($(this).hasClass('team-work')) {
		scrollTo('#team', 1000);
	} else if($(this).hasClass('wallets')) {
...
function adaptation() {
	...
	$('.member-photo img').css({'width': 208 * i,'height':208*i});
	$('.member-name').css({
		'margin-top': 30 * i,
		'margin-bottom': 30 * i,
		'font-size': 20 * i
	});
	$('.member-name b').css({'margin-left':16 * i,'margin-top':10*i});
	$('.member-info').css({
		'width': 267 * i,
		'font-size': 16 * i
	});
	...
	$('#wallet h3 ').css('margin-bottom', 60 * i);
	$('#wallet .intro-logo ').css('margin-right', 104 * i);
