                ...
                <p class="order-list-option">
                  <!-- 仅在已发货状态时显示 上传 按钮 -->
                    ...
                    <template v-if="item.order_state==1">
                     <a class="order-to-detail" href="javascript:;" @click="confirmOrder(item.order_id)">取消</a>
                     <a class="order-sure" href="javascript:;" @click="confirmOrder(item.order_id)">确认</a>
                    </template>                    
...
export default {
  ...
  methods: {
    makeSure(val) {//点击模态框确认最终提交
      console.log(val);
      //根据传递的val(0/1)执行操作
      this.isShow = 0;
    },
    confirmOrder() {//点击确认按钮显示模态框
      this.isShow = 1;
    },
    removeQueryVal() {//删除查询关键字
      this.$emit('removeKey');
      this.value = "";
    },
    toTrack() {//跳转至订单追踪
