<template>
	<div>
		...
		<div class="hitchhiking">
            ...
            {{item.d_name}}                      
            <a v-if="item.da_user_advice==3">待处理</a> 
            <a v-else-if="item.da_user_advice==1">已同意</a> 
            <a v-else-if="item.da_user_advice==2">已拒绝</a> 
          </h3>					
					<div>
						<a :href='"tel:"+item.d_phone'>联系</a>
						<template v-if="item.da_user_advice==3">
            ...
            <img :src="item.d_headurl" alt=""/>
            {{item.d_name}}                      
            <a v-if="item.da_user_is_agree==1">司机申请取消</a>   
            <a v-else-if="item.da_user_is_agree==2">已拒绝司机取消申请</a>  
            <a v-else-if="item.da_user_is_agree==3">已同意司机申请取消</a>  
          </h3>					
					<div>
						<a :href='"tel:"+item.d_phone'>联系</a>
						<template v-if="item.da_user_advice==3">
							<a @click="AgreeCanel(item.da_id,LogisticsDetail.ln_id)" :data-id='item.da_id' :data-lid='LogisticsDetail.ln_id'>同意</a>
	...
	export default {
		...
		methods: {
			...
			AgreeCanel($id, $lid){
				...
				this.post(params, url, this.agreeCanelBack)
			},
			agreeCanelBack(data){
				if(data.status_code==10000){
