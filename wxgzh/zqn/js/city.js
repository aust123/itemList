...
//    JSON.stringify
var cts=(cities.sort(getSortFun('asc', 'pinyin')));
...
for(var i=0;i<cts.length;i++){
    var co=cts[i].pinyin.slice(0,1);
    if(letters.indexOf(co)>=0){
        var index=letters.indexOf(co);
        arr[index].push(cts[i]);
    }
}
for(var l=0;l<arr.length;l++){
    var html='';
    for(var m=0;m<arr[l].length;m++){
        html+='<li>'+arr[l][m].name+'</li>';
    }
    $('.letter:eq('+l+') ul').html(html);
}
