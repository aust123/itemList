    ...
    <div>
        ...
        <section class="search-history">
            <div class="history-content" v-show="!isInput">
                ...
                <p v-for="(item,index) in history" :key="index">{{item}}<img @click="remove(index)" src="../../assets/imgs/sellorder_key_cancel.png" alt=""></p>
            </div>                 
            <p v-for="(item,index) in querydata" :key="index" v-html="item"></p>            
        </section>
    </div>
</template>
<script>
          ...
          res => {           
           let  data = res.s;
            ...
            data.forEach((item,index)=>{
             let newItem = item.replace(val,`<span style='color:red;'>${val}</span>`)
