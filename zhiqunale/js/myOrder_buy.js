...
//					}
    ...
    function dfk(pg,pages){
        ...
        var pathname='/api/paper/get/purchase/order/list';
        GetList(obj,pathname,getList);
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(0)').children('span').html('('+data.data.length+')');
    ...
    function dfh(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(1)').children('span').html('('+data.data.length+')');
    ...
    function dsh(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(2)').children('span').html('('+data.data.length+')');
    ...
    function dys(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(3)').children('span').html('('+data.data.length+')');
    ...
    function djs(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(4)').children('span').html('('+data.data.length+')');
                var html=template('djsList',data);
                $('.djsList').html(html);
                if(pages){
                    $('.djsPages').html('');
                    //分页
                    $('.djsPages').initPagenation({
                        totalCount: data.totalRecord,     //数据总个数,
                        showBtnFirst: true,     //是否显示 首页按钮
                        showCount: 15,          //每页显示多少个,默认10个
                        showBtnsCount: 8      //数字按钮数量,最多10个,默认10个
                ...
                $('.djsList').html(data.msg).css({'display':'block','text-align':'center'});
            }
        }
    }
    $('.djsPages').delegate('a','click',function(){
        djs($(this).html(),false);
    });
    $('.djsPages').delegate('.pre-page','click',function(){
        var page=$(this).siblings('a.active').html();
        if(page!=1){
            djs(page-1,false);
        }
    });
    $('.djsPages').delegate('.next-page','click',function(){
        var page=$(this).siblings('a.active').html();
        if(page!=$(this).prev().prev().html()){
            djs(parseInt(page)+1,false);
        }
    });
    $('.djsPages').delegate('.first-page','click',function(){
        var page=$(this).next().html();
        djs(page,false);
    });
    $('.djsPages').delegate('.last-page','click',function(){
        var page=$(this).prev().html();
        djs(page,false);
    });
    //已完成
    ...
    function ywc(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(5)').children('span').html('('+data.data.length+')');
                ...
                $('.ywcPages').html('');
                $('.states li:eq(5)').children('span').html('(0)');
    ...
    function tk(pg,pages){
        ...
        function getList(data){
            if(data.status_code=='10000'&&data.data.length!=0){
                $('.states li:eq(6)').children('span').html('('+data.data.length+')');
                var html=template('tkList',data);
                $('.tkList').html(html);
                if(pages){
                    $('.tkPages').html('');
                    //分页
                    $('.tkPages').initPagenation({
                        totalCount: data.totalRecord,     //数据总个数,
                        showBtnFirst: true,     //是否显示 首页按钮
                        showCount: 15,          //每页显示多少个,默认10个
                        showBtnsCount: 8      //数字按钮数量,最多10个,默认10个
                ...
                $('.tkPages').html('');
                $('.states li:eq(6)').children('span').html('(0)');
                $('.tkList').html(data.msg).css({'display':'block','text-align':'center'});
            }
        }
    }
    $('.tkPages').delegate('a','click',function(){
        tk($(this).html(),false);
    });
    $('.tkPages').delegate('.pre-page','click',function(){
        var page=$(this).siblings('a.active').html();
        if(page!=1){
            tk(page-1,false);
        }
    });
    $('.tkPages').delegate('.next-page','click',function(){
        var page=$(this).siblings('a.active').html();
        if(page!=$(this).prev().prev().html()){
            tk(parseInt(page)+1,false);
        }
    });
    $('.tkPages').delegate('.first-page','click',function(){
        var page=$(this).next().html();
        tk(page,false);
    });
    $('.tkPages').delegate('.last-page','click',function(){
        var page=$(this).prev().html();
        tk(page,false);
    });
//	选择开始及结束时间
