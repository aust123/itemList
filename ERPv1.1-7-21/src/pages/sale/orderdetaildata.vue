    ...
    <div class="order-detail-content">
      ...
      <template v-if="orderType==1">
          ...
          <h3 class="item-title">货品信息<span>{{orderDetailData.order_create_time|setDateFormat}}</span></h3>
            <section class="detail-info">
              ...
              <p>订单重量：<span>{{orderDetailData.product_weight}}</span><b class="unit">吨</b></p>
              <p>货品单价：<span>{{orderDetailData.per_price}}</span><b class="unit">元/吨</b></p>
              <p>订单金额：<span class="total">￥{{orderDetailData.send_total_money}}</span></p>
            </section>
            ...
            <template v-if="orderDetailData.order_state != 1 && orderDetailData.order_state != 2 && orderDetailData.order_state != 3 && orderDetailData.order_state != 4">
              ...
              <p>结算总额：<span>￥{{orderDetailData.settle_total_money-orderDetailData.settle_total_deductions}}</span></p>
              <p>实收金额：<span class="total">￥{{orderDetailData.real_gather}}</span></p>
              </section>
            </template>
        <!-- 物流信息在待出库状态不显示 -->
      ...
      <template v-else>
            ...
            <section class="detail-info">
              ...
              <p>采购重量：<span>{{orderDetailData.purchase_weight}}</span><b class="unit">吨</b></p>
              <p>采购单价：<span>{{orderDetailData.per_price}}</span><b class="unit">元/吨</b></p>
              <p>采购金额：<span class="total">￥{{orderDetailData.total_money}}</span></p>
            </section>
      <!-- 结算信息在待出库、待发货、已发货状态不显示 -->
            <template v-if="orderDetailData.order_state != 1 && orderDetailData.order_state != 2 && orderDetailData.order_state != 3 && orderDetailData.order_state != 6">
              ...
              <p>其他扣除：<span>￥{{orderDetailData.settle_total_deductions}}</span></p>
               <p>结算总额：<span>￥{{orderDetailData.settle_total_money-orderDetailData.settle_total_deductions}}</span></p>
              <p>实收金额：<span class="total">￥{{orderDetailData.real_pay}}</span></p>
              </section>
            </template>
        <!-- 物流信息 -->
              <template >
              ...
              <section class="orderdetail-info">
                ...
                <template v-else >
                  <p>取消时间：{{orderDetailData.cancel_time}}</p>
                  <p>操作人：{{orderDetailData.cancel_operator}}</p>
                  <p v-if="orderDetailData.cancel_reason">取消原因：{{orderDetailData.cancel_reason}}</p>
                  <p v-else>取消原因：暂无</p>
                </template>
              </section>
        <!-- 订单在待确认状态时 确认/取消 操作按钮 -->
            <template v-if="orderDetailData.order_state==1">
              <p class="detail-option"><a class="cancel" href="javascript:;" @click="confirmOrder(orderDetailData.order_id,0)">取消</a><a class="cancel" href="javascript:;">修改</a><a  href="javascript:;" @click="confirmOrder(orderDetailData.order_id,1)">确认</a></p>
            </template>
        <!-- 订单在已发货状态时显示  上传结算单 操作按钮 -->
