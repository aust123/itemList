  ...
  methods: {
    changeType() {
      ...
      let params = {
        ...
        order_type: this.orderType //订单类型
      };
      //在切换检索类型时获取检索历史记录
      api.getSearchInfo(params).then(
        res => {
          if (res.status_code == 10000) {
    ...
    query(event) {
      this.querydata = [];
      let val = event.target.value || this.KeyValue;
       this.isInput = true;          
      if (val) {
        if (this.BuySaleSearchInfo.length > 0) {               
          ...
          this.tipMsg = ["未搜到相关数据", true];
        }
      } else {
        this.tipMsg = ["请输入关键字", true];
      }
    }
  }
};
