...
<div>
    ...
    <section id="login">			
        ...
        <div><input :class="{waring:nopwd}" type="password" name="" id="pwd" placeholder="密码" v-model="password" @keydown.enter="login"/></div>
        <div><input type="submit"  id="btn" value="登录" @click="login" @keydown.enter="login"/></div>	
    </section> 
    ...
    login() {     
      ...
      if (this.phone && this.password) {
        ...
        api.Login(params).then(res => {                 
          ...
          this.nopwd=1         
           this.tipMsg=['请输入密码',true];
      }else if(!this.phone&&this.password){
          this.noname=1         
           this.tipMsg=['请输入手机号',true];
      }else{
          this.nopwd=1;
          this.noname=1;         
          this.tipMsg=['请输入完成信息',true];
      }
    }
  }
};
