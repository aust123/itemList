<template>
  <div class="billing">
    ...
    <form class="billForm divItem">
      <div class="form_item">
        <span>仓库</span>
        <input class="input1" v-model="ckVal" readonly type="text" placeholder="请输入仓库名称"/>
      </div>
      <div class="form_item">
        <span>货位</span>
      ...
      <div class="form_item">
        <span>出库重量</span>
        <em>吨</em>
        <input class="input3" v-model="chuku" type="text"/>
      </div>
    </form>
    <div class="account divItem">
      <div class="h152">
        <a @click="sure" class="sure">确认</a>
        <a @click="del" class="delete">删除</a>
      </div>
    </div>
  ...
  export default{
    ...
    methods:{
      ...
      sure:function(){
        ...
        }else if(!this.checkWeight(this.chuku)){
