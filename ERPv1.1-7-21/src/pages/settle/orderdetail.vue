<template>
    <div>
        ...
        <p class="order-list-title">
          ...
          <span class="status" v-if="settle_data.es_state==1">待确认</span>
          <span class="status" v-else-if="settle_data.es_state==2">已确认</span>
      ...
      <h3 class="item-title">质检信息</h3>
        <section class="settle-info  nopadding">
          <table>
        ...
        <section class="settle-info">
          <table>
            ...
            <tr class="table-content">
              ...
              <td>{{(parseFloat(settle_data.es_settle_watery)+parseFloat(settle_data.es_settle_trash)).toFixed(3)}}</td>
              <td>{{settle_data.es_per_price}}</td>
              <td>{{settle_data.es_pay_money}}</td>
            </tr>
          </table>
       ...
       <p style="padding-bottom:10px">结算总额：<span class="total">￥{{order_data.settle_all_price}}</span></p>
        </section>
        ...
        <p class="detail-option" v-if="settle_data.es_state==1" ><router-link  :to="{path:'/accountSure',query:{oid:settle_data.es_order_id,sid:sid,path:$route.fullPath}}">结算</router-link></p>
       </div>
    </div>
</template>
<script>
