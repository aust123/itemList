<template>
    ...
    <div class="order-detail-content">
      ...
      <section class="detail-info">
        ...
        <p class="detail-option" v-if="sendDetailInfo.ego_state==1"> <router-link :to="{path:'/delivery',query:{oid:sendDetailInfo.ego_id,path:$route.fullPath}}" class="order-sure"  >发货</router-link></p>
  ...
  methods: {
    ...
    api.getSendOrderDetail(params).then(res => {
      ...
      if (res.status_code == 10000) {
         ...
         res.data.logistics_info.forEach((item,index) => {
          this.totalCount += parseInt(item['eli_entruck_amount']);
        });
      }
    });
    }
