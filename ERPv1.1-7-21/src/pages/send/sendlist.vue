         ...
         <div class="content">
            ...
            <section class="orderList" v-for="(item,index) in sendData" :key="index">
                ...
                <p class="order-list-option">
                     <router-link :to="{path:'/delivery',query:{oid:item.ego_id,path:$route.fullPath}}" class="order-sure"  v-if="item.ego_state==1">发货</router-link>
    ...
    getdata() {
      ...
      api.getSendListAndSearch(params).then(res => {
        ...
        if (res.status_code == 10000) {
          this.sendData = [...this.sendData,...res.data];
          if (this.sendData.length % this.size != 0) {
            this.allLoaded = true;
          }
