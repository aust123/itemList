<template>
  <div class="publish_supply">
    ...
    <ul class="address_list">
      <li v-for="item in addresslist" :class="{active:item.a_is_default==1}">
        <h3>{{item.address}}</h3>
        <h3>
        ...
        <p>
          <a @click="editAddr($event)" class="edit"
             :data-a_id="item.a_id"
             :data-a_username="item.a_username"
             :data-a_phone="item.a_phone" :data-province="item.a_province"
             :data-city="item.a_city" :data-dist="item.a_dist"
             :data-detail="item.a_detail"
            :data-moren="item.a_is_default"
            :data-lng="item.a_lng"
            :data-lat="item.a_lat">编辑</a>
