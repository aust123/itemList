...
export default {
  ...
  created() {
    ...
    api.getBalanceSearchType(params).then(res => {
      console.log(res);
  ...
  methods: {
    changeType() {
      ...
      //在切换检索类型时获取检索历史记录
      api.getBalanceHistoryInfo(params).then(res => {
        console.log(res);
        if (res.status_code == 10000) {
    ...
    removeData(isAll, i) {
      ...
      api.clearBalanceHistorySearchList(params).then(res => {
        console.log(res);
        if (res.status_code == 10000) {
          this.history = isAll
            ? []
    ...
    toList($item) {
      ...
      //console.log( this.cookie.getCookie('keywords'))
      this.$router.push({
        path: "/checklist"
      });
    },
    query(event) {
      ...
      this.querydata = [];
      api.getBalanceSearchInfo(params).then(res => {
        if (res.status_code == 10000) {
