<template>
    <div>
        <Header :noSearch='0'><span slot="title">质检单详情</span></Header>
      ...
      <section class="detail-info">
        <p class="order-list-info"><img src="../../assets/imgs/orderdetail_img.png" alt=""><b>{{order_data.epo_product_stage}}</b><b>{{order_data.epo_product_name}}</b><span>{{order_data.epo_supplier_name}}</span></p>
       ...
       <p>扣除原因：<span v-if="quality_data.eqc_other_deductions_remark">{{quality_data.eqc_other_deductions_remark}}</span><span v-else class="nodatatips">暂无</span></p>
        ...
        <p class="detail-option" v-if="quality_data.eqc_state==1"><router-link  to="/inspection" class="jiesuan">质检录入</router-link></p>
       </div>
    </div>
</template>
<script>
