<template>
	<div class="footer">
		<ul class="footer-list">
			<li class="item" v-for="(item,index) in footerList" :class="{'active':nowIndex===index}" @click="tabClick(index)">
				{{item.name}}
			</li>
		</ul>
	</div>
...
<script>
	export default {
		...
		methods: {
			initPage: function() {
				this.nowIndex = this.$route.path === '/buy' ? 0 : this.$route.path === '/sale' ? 1 : this.$route.path === '/message' ? 2 : this.$route.path === '/order' ? 3 : this.$route.path === '/my' ? 4 : 0
			},
